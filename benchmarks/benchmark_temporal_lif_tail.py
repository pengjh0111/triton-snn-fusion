...
def ref_tail(x, v, w, b):
    return snn_custom_ops.fused_temporal_lif_tail_torch(x, v, w, b, 1.0, 0.0, 2.0, False)
...
def run_case(T, shape, args):
    ...
    compiled = torch.compile(lambda a, vv, ww, bb: ref_tail(a, vv, ww, bb), fullgraph=False, dynamic=False)
    snn_custom_ops.configure_fused_op("triton", strict_triton=args.strict_triton, verbose=args.verbose)

    def eager_fn():
        return ref_tail(x, v, w, b)

    def compile_fn():
        return compiled(x, v, w, b)

    def triton_fn():
        return torch.ops.snn_custom.fused_temporal_lif_tail.default(x, v, w, b, 1.0, 0.0, 2.0, False)
    ...
    allclose = torch.allclose(ref[0], got[0], atol=atol, rtol=rtol) and torch.allclose(ref[1], got[1], atol=atol, rtol=rtol)
    max_err = max((ref[0] - got[0]).abs().max().item(), (ref[1] - got[1]).abs().max().item())
    ...
    return {
        ...
        "temporal_lif_tail_triton_ms": tri["mean_ms"],
        ...
        "triton_hit": stats.get("temporal_lif_tail_triton", 0),
        "fallback": stats.get("temporal_lif_tail_fallback", 0),
...
def parse_args():
    ...
    parser.add_argument("--out-dir", default="temporal_lif_tail_benchmark")
...
def main():
    ...
    print("| T | shape | dtype | eager | compile | triton | speedup compile | allclose | hit | fallback |")
    print("|---:|---|---|---:|---:|---:|---:|---|---:|---:|")
    for r in rows:
        print(f"| {r['T']} | {r['shape']} | {r['dtype']} | {r['eager_loop_ms']:.3f} | {r['compile_loop_ms']:.3f} | {r['temporal_lif_tail_triton_ms']:.3f} | {r['speedup_vs_compile']:.3f} | {r['allclose']} | {r['triton_hit']} | {r['fallback']} |")
    out = Path(args.out_dir)
    out.mkdir(parents=True, exist_ok=True)
    (out / "temporal_lif_tail_benchmark.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
