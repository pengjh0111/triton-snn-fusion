...
@dataclass
class TritonTemporalLIFAvgPoolLinearResult:
    out_sum: torch.Tensor
    v_next: torch.Tensor
    used_triton: bool
    kernel_key: str = "temporal_lif_avgpool_linear"
    fallback_reason: str = ""
...
def run_triton_fused_temporal_lif_avgpool_linear(
    x_seq,
    v_init,
    fc_weight,
    fc_bias,
    ...
    if reasons:
        if verbose:
            print(f"[TRITON][FALLBACK][temporal_lif_avgpool_linear] {'; '.join(reasons)}")
        if strict:
            raise RuntimeError("[TRITON][STRICT][temporal_lif_avgpool_linear] " + "; ".join(reasons))
        raise RuntimeError("; ".join(reasons))
    try:
        from kernels.generated_temporal_lif_avgpool_linear_kernel import run_fused_temporal_lif_avgpool_linear_kernel

        out_sum, v_next = run_fused_temporal_lif_avgpool_linear_kernel(
            x_seq,
            v_init,
            fc_weight,
            fc_bias,
        ...
        return TritonTemporalLIFAvgPoolLinearResult(out_sum, v_next, True, kernel_diagnostics=diagnostics)
    except Exception as exc:
        if verbose:
            print(f"[TRITON][FALLBACK][temporal_lif_avgpool_linear] Triton kernel call failed: {exc}")
        if strict:
            raise
        raise RuntimeError(f"temporal LIF avgpool linear Triton kernel call failed: {exc}") from exc


# Deprecated compatibility aliases.
TritonTemporalLIFTailResult = TritonTemporalLIFAvgPoolLinearResult
strict_temporal_lif_tail_enabled = strict_temporal_lif_avgpool_linear_enabled
check_temporal_lif_tail_support = check_temporal_lif_avgpool_linear_support
run_triton_fused_temporal_lif_tail = run_triton_fused_temporal_lif_avgpool_linear
